...
def get_contact_points(gripper):
    Panda_tip_pose = Dummy('Panda_tip').get_pose()
    Panda_leftfinger_respondable_pose = Shape('Panda_leftfinger_respondable').get_pose()
    Panda_leftfinger_visual_pose = Shape('Panda_leftfinger_visual').get_pose()
    Panda_rightfinger_respondable_pose = Shape('Panda_rightfinger_respondable').get_pose()
    Panda_rightfinger_visual_pose = Shape('Panda_rightfinger_visual').get_pose()
    dir_vec_visual = Panda_leftfinger_visual_pose[:3] - Panda_rightfinger_visual_pose[:3]
    dir_vec_visual = dir_vec_visual / np.linalg.norm(dir_vec_visual)
    dir_vec_respondable = Panda_leftfinger_respondable_pose[:3] - Panda_rightfinger_respondable_pose[:3]
    dir_vec_respondable = dir_vec_respondable / np.linalg.norm(dir_vec_respondable)
    gripper_joint_positions = gripper.get_joint_positions()
    cp1 = Panda_tip_pose[:3] + dir_vec_visual * gripper_joint_positions[0]
    cp2 = Panda_tip_pose[:3] - dir_vec_visual * gripper_joint_positions[1]

    return np.array([cp1, cp2, Panda_tip_pose[:3]])

def get_end_point(task):
    end_points = []
    for condition in task._success_conditions:
        if hasattr(condition,'_obj'):
            condition_obj_pose = condition._obj.get_pose()
            end_points.append(condition_obj_pose[:3])
        else:
            pass
...
class Scene(object):
    ...
    def get_demo(self, record: bool = True,
            ...
            for i, point in enumerate(waypoints):
                ...
                if len(ext) > 0:
                    ...
                    if 'close_gripper(' in ext:
                        ...
                        cp_list = get_contact_points(gripper)
                        cp_info = {'cp_list': cp_list, 'cp_frame': len(demo)}
                    self._demo_record_step(demo, record, callable_each_step)
        ...
        if not success:
            ...
            end_points = get_end_point(self.task)
        processed_demo = Demo(demo)
        processed_demo.num_reset_attempts = self._attempts + 1
        return processed_demo, cp_info, end_points
