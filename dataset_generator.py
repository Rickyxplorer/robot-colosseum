...
def run(
    ...
    for ex_idx in range(ex_start, data_cfg.episodes_per_task):
        ...
        while attempts > 0:
            try:
                # TODO: for now we do the explicit looping.
                (demo,), cp_info, end_points = task_env.get_demos(amount=1, live_demos=True)
            except Exception as e:
                attempts -= 1
                if attempts > 0:
                    continue
            ...
            with file_lock:
                ...
                np.save(os.path.join(episode_path, 'cp_list.npy'), cp_info['cp_list'])
                np.save(os.path.join(episode_path, 'frame.npy'), cp_info['cp_frame'])
                np.save(os.path.join(episode_path, 'end_points.npy'), end_points)
                if save_state is not None:
                    save_state.number_episodes += 1
                    with open(save_state.save_path, "wb") as fhandle:
                        pickle.dump(save_state, fhandle)
